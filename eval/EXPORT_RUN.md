...
| 2026-09-05 | **eval_core v1.9 — the bar is class per condition per phase.** NEW perPhaseClassAgreement ≥0.90 (rows expanded to phases; "All phases" = profile; a phase named twice takes the worst class; abstentions never count as agreement; byPhase + worstConditions reported). severityAgreement (strict rows) and phaseSplitAgreement become INFORMATIONAL | Waqas, 5 Sep: "same class different effect we just ruled doesnt need to be on the same row" — one Major row for Takeoff+Climb vs two Major rows with different effects is the same answer; row pairing and split matching punished the presentation. Draws e1/e2/e3 (76.59 + fha_derive 1.1): per-phase class 0.71 / 0.66 / 0.63, function worst case 0.91 / 0.96 / 0.96 |
| 2026-09-05 | **eval_core v1.8 — lever 4.** NEW phaseSplitAgreement ≥0.90 (conditions both runs drafted whose rows split the flight into the same phase groups) with a per-condition `flips` report (split moved / class moved on a paired row) | Identical-input draws 2 vs 3 gave the same row count on 74 of 107 conditions; a build aimed at consistency needs to see WHICH conditions move, not the average |
