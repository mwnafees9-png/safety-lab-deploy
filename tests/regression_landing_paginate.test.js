...
check('gradient identity: one continuous body wash, sections transparent, dark scheme', /linear-gradient\(180deg, #14224A 0%, #3457A2 45%, #7247B1 85%, #9B55BE 100%\)/.test(s) && /background: transparent !important/.test(s) && /color-scheme: dark/.test(s));
...
check('More dropdown is an opaque panel, not glass', /\.nav-more-menu\s*\{[^}]*background:\s*rgba\(11,\s*20,\s*46,\s*0\.97\)\s*!important/.test(s));
...
check('glass surfaces + light clipped headline on dark', /backdrop-filter: blur\(10px\)/.test(s) && /#BFDBFF/.test(s));
