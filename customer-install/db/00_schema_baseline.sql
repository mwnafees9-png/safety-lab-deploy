...
CREATE OR REPLACE FUNCTION private.is_safety_lab_admin()
 ...
 SET search_path TO 'public'
AS $function$
  -- Customer install: no platform admins baked in. Migration 02 replaces this
  -- with a lookup against private.platform_admins (which the operator seeds).
  select false;
$function$
;
...
begin
  ...
  return jsonb_build_object(
    'error', jsonb_build_object(
      'http_code', 403,
      'message', 'Your Microsoft organization is not yet approved. Contact your Safety Lab administrator to enable SSO for your tenant.'
    )
  );
end;
$function$
