...
| 4 | **Desktop parity** *(added 6 Sep — Waqas: "the 4th on the urgent 3 map")* | Web and desktop are not in sync on ANY axis: the desktop bundle is a stale hand-pasted copy (~25% of the product missing), it has NO accounts (a "user" is a typed-in name, default `Desktop User`), it grants Pro+ by default with a fake token, and by default talks to no backend. Fix = one account honored by both apps living in the CUSTOMER's sign-in; desktop ships the same web bundle pulled at build time; desktop signs in for real through the config surface and syncs via the customer's backend. Makes "notify me on sign-in" and the paywall real. Sequence: offline license (serves both apps) → desktop parity → collaboration (built once, both get it). |
| 5 | **MFA** | Broken, not a policy choice. Fix it and turn it on. |
