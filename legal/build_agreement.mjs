 ...
 *   site/legal.html                       (the agreement rendered statically between EULA markers)
...
// ---- 3b. the public /legal page ------------------------------------------------------------------
// Rendered statically so the agreement is on the page itself (search engines, screen readers, scripts
// off), between markers this generator owns. The modal and the page can no longer drift apart.
{
  const LEGAL = join(REPO, 'site', 'legal.html');
  const B = '<!-- EULA:BEGIN (generated by legal/build_agreement.mjs from legal/SL-EULA-0004.html, do not hand-edit) -->';
  const E = '<!-- EULA:END -->';
  const cur = readFileSync(LEGAL, 'utf8');
  const a = cur.indexOf(B), b = cur.indexOf(E);
  if (a < 0 || b < 0) problems.push('site/legal.html has no EULA markers');
  else {
    const rev = VERSION.replace(/-([A-Z0-9]+)$/, ' &middot; Rev $1');
    const want = (cur.slice(0, a + B.length) + html + cur.slice(b))
      .replace(/(<p class="docmeta" id="eula-meta">)[^<]*(<\/p>)/, '$1' + rev + '$2');
    if (cur !== want) {
      if (check) problems.push('site/legal.html agreement text is stale');
      else { writeFileSync(LEGAL, want); wrote.push('site/legal.html'); }
    }
  }
}
