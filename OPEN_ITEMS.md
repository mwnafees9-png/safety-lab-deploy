...
- **R16 — Prompt caching BUILT 12 Sep (afternoon), f7a8c00 web + 23ae01c proxy (item 16 of NEXT_SESSION).** Byte-identical packaging: the assembler records two cache breaks (after the skill body, after the source document), AiClient cuts the string into marked blocks on the wire; proxy meters reads 0.10x / writes 1.25x and now weights claude-opus-4-8 at 5x (it was missing → metered at 1x). Pending: `./ship.sh`, proxy `./deploy.sh`, the two-draft live proof (audit log `cacheRead`). Follow-up decision: the stable-first REORDER variant (bigger hit rate on the chat/batch path, eval-gated ~$5) once the Console shows the measured hit rate.
