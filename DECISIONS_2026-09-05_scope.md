...
8. **Desktop parity is the 4th urgent build** ("this needs to be the 4th on the urgent 3 map"). Confirmed from the desktop code: no shared identity (desktop has no accounts — a user is a typed-in name), stale hand-pasted bundle (~25% of the product missing), Pro+ granted by default with a fake `desktop-local` token, no backend by default. Fix: one account in the customer's sign-in honored by both apps; same web bundle pulled at build; real desktop sign-in through the config surface; sync via the customer's backend. Sequence: offline license → desktop parity → collaboration.
9. **Offline signed license — "most robust infrastructure we can establish."** Ed25519-signed license file: id, customer, tier, seats (attested cap enforced by the customer's own sign-in), feature flags, notBefore/notAfter (time-boxed = revocation), bound to the customer's SSO tenant + email domains; multiple public keys for rotation; clock-tamper resistance; versioned format; one format for self-hosted web / desktop / browser-only. Private signing key custody: a Node tool Waqas runs himself; Claude only ever sees the public key. Demo cloud keeps the DB-row path. Open: whether a defense customer requires a FIPS algorithm instead of Ed25519.
