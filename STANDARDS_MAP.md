...
| 4.6.4 | Aircraft/system integration and verification | A strategy to look for unintended behaviour, with a list of test stimuli | Covered 23 Sep (G6): `ub_stimuli.js` |
...
| 5.7 | Process assurance | PA plan, reviews, data; issue/finding/action item (App E.7) | Covered 23 Sep (G7): `pa_audit.js` |
...
| G3 | PRA requirement cascade (origin, rationale, allocation, interrelation flag) and PRA assumptions with impact are not first-class | 4761A Q.15.2.7.2, L | **Done 23 Sep.** `pra_scenarios.js`: per risk, scenarios (hit together → aircraft effect → linked FCs → classification → acceptable + rationale) and assumption impacts; one traced requirement per scenario (`pra-scenario`: origin, rationale, allocation to owning systems, interrelated risks); INV-53 advisory. Risks without scenarios keep the `pra-zonal` requirement. Fixed on the way: form saves dropped fields the form does not show (PRA canvas links/zones, auto-requirement sources); the PRA import wrote fields the PRA tab never reads. `regression_pra_scenarios.test.js` |
...
| G5 | No USOC workflow | F3061 4.2.6, X2 | **Done 23 Sep.** `usoc.js`: candidates found from FCIM aware/unaware pairs where the aware row governs and the unaware outcome escalates (Major-or-less → Haz/Cat, or Haz → Cat), or added by hand; dispositioned (USOC / not, with reason); per USOC: crew information (inherent or F3117 alert), action, time available, procedure (airmanship / AFM), clarity evidence, and three substantiation calls; one `usoc-info` requirement each (§4.2.6); INV-54 advisory; CSV export. On the AFHA menu. `regression_usoc.test.js` |
| G6 | No integration-level strategy for unintended behaviour (stimuli list, results in the verification summary) | 4754B 4.6.4 | **Done 23 Sep.** `ub_stimuli.js`: a strategy (approach, why it is enough, owner) and a list of stimuli STIM-n (abnormal input, failure injection, boundary or timing, mode transition, combined failures; target aircraft or a system; pass criteria). Suggestions are read from the model (basic events under Cat/Haz trees, flight phase changes, FCIM functions) and added only when accepted. Results per stimulus: run date, observed, unintended behavior none/found, evidence, problem report. The results join the verification matrix in every report (aircraft: all; a system report: its own). INV-56 advisory; Appendix A row A5-4; opened from the Requirements Repository page, CSV export. `regression_ub_stimuli.test.js` |
| G7 | No PA audit objects (issue / finding / action item, sampling) | 4754B 5.7, E.7 | **Done 23 Sep.** `pa_audit.js`: a PA plan (owner, independence, default sampling, cadence); audits PAA-n (date, auditor, independence, the ARP4754B process area, scope); a repeatable sample of the project's own records for that area (FCs, requirements, requirements with a verification method, problem reports), drawn at random, risk-first (every Cat/Haz item) or in full, with the seed saved so anyone can redraw it; issues, findings and action items per audit (owner, due, closure evidence, problem report link). INV-55 advisory: no owner, no audits, not independent at DAL A/B, unsampled or drifted samples, a finding with no action, overdue or ownerless actions, closed with no evidence. Appendix A row A7-4; opened from the Appendix A page, CSV export. `regression_pa_audit.test.js` |
