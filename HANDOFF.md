...
**STILL PENDING A RULING.** ~80 MB of untracked scratch (`_bak` 34 MB, `backups-2026-07-{11,12,14,15}` 26 MB, `_to_delete` 18 MB, `_claude_stage`, `Claude outputs`). None is in git history and none can break a build. Held back because the inventory records that `_bak` contains the 4 Sep data-loss investigation states — that is measurement-adjacent, and rule 26 says read and archive before anything goes. Waqas to rule.
