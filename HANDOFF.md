...
1. **`private.owns_workspace(uuid)`** — the eighth helper. SECURITY DEFINER, pinned search_path, `coalesce`d so it can never return NULL (the 20260831c lesson), revoked from anon/public. It reads `workspaces.owner_id` rather than a role **because at the moment of creation the creator is not yet a member** — `is_workspace_owner()` reads `workspace_members` and would answer false for the very row being inserted.
2. **`workspaces_owner_read`** — an owner can always SELECT their own workspace. Between creating the workspace row and inserting the first member row, `workspaces_member_read` (which requires membership) blocks the creator from seeing what they just made; the client's fallback path does `insert(row).select('id').single()`, i.e. a RETURNING, which needs SELECT — **so that path is broken today** and nobody noticed, because the primary path generates the id client-side.
3. **`ws_members_self_join` fixed** — `auth.uid() = user_id AND private.owns_workspace(workspace_id) AND role = 'owner'`. Three conditions, all ANDed. Every other way in goes through `accept_invitation`, which is SECURITY DEFINER and already enforces email match, expiry, single use, no downgrade and a role whitelist that excludes owner.
4. **`ws_members_admin_update`** gains a WITH CHECK (it had none, which is how an admin could set their own role to owner); both USING and WITH CHECK now exclude `role = 'owner'`, so an admin can neither edit the owner's row nor promote anyone to owner. Deliberate consequence: promotion to owner is impossible by ordinary update for ANYONE, including the owner.
5. **`ws_members_admin_delete`** excludes the owner's row — Waqas: "Admins should not be able to evict the owner, unless ownership has been transferred." A DELETE policy takes no WITH CHECK, but USING is evaluated against the row being deleted, which is exactly what is needed.
6. **`public.transfer_workspace_ownership(uuid, uuid)`** — the one sanctioned route. Owner-only, atomic, and **the new owner must already be a member**, which means ownership can only ever reach someone who came in through an invitation. Plus the **`erase_project` grant drift** corrected in the same file (0005:43 grants it to `authenticated`; production grants only service_role — the DISK is more permissive than production, and a rebuild would hand a destructive RPC to every signed-in user).

**THE AUDIT SUITE THAT MISSED S1 IS WIDENED, AND IT IMMEDIATELY FOUND MORE.** `regression_h8_rls_disk_sync` only considered 0001 policies whose line named one of the seven helpers, because its job was the private-schema qualification; `auth.uid() = user_id` names no helper, so the one broken policy was invisible BY CONSTRUCTION and stayed so through three reviews. It now accounts for **every** policy 0001 defines. That surfaced six more never re-emitted — `users_self_read`, `ai_usage_self_read`, `audit_self_read`, `license_tokens_self_select`, `workspaces_self_insert`, `feedback_insert_own` — each read against a stated test and allowlisted with its reason. **The test for the allowlist, and it is the one S1 failed:** a policy may survive from 0001 only if it is scoped to the caller's own row AND granting it cannot give the caller access to another tenant's data. `ws_members_self_join` passed the first half and failed the second — the row it inserted WAS a grant of access. A second check now refuses to let the allowlist ever excuse an INSERT on a grant table; mutation-proven by trying to add `ws_members_self_join` to it (goes red).

**`tests/regression_workspace_membership.test.js` — 30 checks, mutation-proven six ways:** re-open S1 by dropping the ownership condition → red; drop the role condition → red; let admins edit the owner row → red; let admins evict the owner → red; leave `erase_project` granted to authenticated → red; drop the new-owner-must-be-a-member check → red. Its `policyBody()` helper had to be rewritten mid-build: the first draft matched the OLD BROKEN POLICY quoted in the migration's own header comment and reported an empty body, failing three checks against a correct migration. **A probe that reads the defect it is hunting and calls it the current state is worse than no probe** — it now anchors only on lines that BEGIN a real statement.

**STATIC, AND SAID SO IN THE FILE.** The wall runs with no database, so these checks read the migration, not the server. The live proof is the post-apply checklist at the foot of the migration.

**WALL: 275 suites, 2 real fails, 0 crashed** — both the environment-only proxy-sibling checks; 0 on Waqas's machine.

**NOT APPLIED. Waqas's next actions, in order:** (1) run the PRE-FLIGHT query at the foot of the migration and keep the result — it lists membership rows with no owner and no redeemed invitation behind them, i.e. anyone who already walked through the open door, which this migration does NOT remove; (2) apply the migration; (3) work the POST-APPLY checklist (a)–(h). No app code changed, so no `ship.sh` is needed for this build.

## 05 Sep 2026 (night) — the old roadmap scrapped (doctrine rescued first), the CoFFE upgrade read against the standard and scheduled last, the full build order settled.

**ROADMAP_Process_Layer.md IS GONE** (Waqas: "scrap the road map", clarified as the old one, not tonight's build plan). Written 2 Jul 2026 against v61.00; its PHASE LIST had gone stale enough to be dangerous — the interdependence table, MAC editor, compiler, CoFFE and equivalence verifier had all shipped without it being updated, and a plan listing built things as unbuilt sends the next person to build them twice. **Its ten governing design decisions were moved to the top of `OPEN_ITEMS.md` before deletion**, because that file was their only home and they are still the operating doctrine: probability-only allocation; artifacts are renderings, never hand-maintained; fixed objectives with pluggable methods and recorded tailoring; MAC + Interdependence as the aircraft-level model with trees/cut sets/CoFFE verdicts/budgets as compiled OUTPUTS; two-lane discipline (computed checks elicited, neither overwrites the other); **AI drafts, the core proves, AI never originates a number**; Independence Principles as a deduped claim registry; cockpits not report generators; baseline-and-hand-off; Q-format reports. The parked list went with them. `ARP4761A_4754B_Gap_Analysis.md` was the only file pointing at the roadmap and now points at the register.

**RECORDED AS LEADS, NOT FINDINGS.** A 5 Sep name search suggested three roadmap items are absent — the status engine, assumption routing, and the Independence Principle ledger (decision 7, which the standard leans on for proposing independence requirements). Written into the register as **unconfirmed**: a name search proving absence is the exact trap rule 19 exists for (the "0 transfer gates" probe that was wrong). Confirm by more than one name before building.

**B8 — THE CoFFE UPGRADE, READ AGAINST THE STANDARD AND SCHEDULED LAST.** ARP4761A B.4.3.1 (p.56, Table B2) and the worked example Q.4.4.1 (pp.382-387, Tables Q.4-6/7/8). The method: aircraft FC → contributing system functions from the Interdependence Analysis → enumerate their states in combination → record the capability → answer whether each produces the condition. The braking example runs 4 functions x 3 states (Failed/Degraded/Operational) = 81 cases, then **Q.4-7 reduces to the minimum contributors — six lines, and that reduced set IS the product**: it becomes the AND gate in the fault tree and is where Independence Principles get proposed.

**Two things the standard CONFIRMS about what is already built:** malfunction correctly sits outside the availability arithmetic (in the example, uncommanded high thrust is not in the CoFFE table at all — already a Catastrophic system-level FC, so it hangs off the top OR gate and goes to the propulsion PSSA, which is exactly why `coffeComputed` returns null for malfunction cases); and "degraded" is a per-project ASSUMPTION, declared in the example as half capability (PASA-ASMP-02), not a universal.

**NOT re-proposed: a third STATE token.** B7 measured it and it was withdrawn — 676 cases to ~1,500 with a computed lane for none, because partial loss is not a state but a capability below full. That reasoning stands. **But the standard does need degraded** (the summary line is "total loss of wheel brake in addition to PARTIAL loss of any ground spoiler or thrust reverser or flap"), and the representation ALREADY EXISTS and is empty: `rule.degraded[].weight` + clause `floor`, used by **0 of 22 rules**. Same data-entry job as F23, not a new mechanism. And depth is not solved by brute force — Q.4-7 is a minimum-contributor set, which is what a minimal-cut-set computation over the MAC model produces (design decision 4).

**The B8 work:** wire `coffeCoverage` (built, zero callers, reports what the sweep left out — today nothing tells a user triples were never enumerated, against the house rule that a bounded sweep says what it left out); leave `coffeShortestRoute` OFF pending Waqas's ruling (it raises a hard finding on a genuine single-point failure and could open the demo showing unrehearsed findings — a demo-risk call, his); make the AI residue lane record its failure reasons (run 3: 101 calls, 44 failed, no reason); cap the residue (1,389 cases on run 2); populate the degraded weights with F23's data work; cross-check CoFFE against the SFHAs once they exist, which the standard asks for explicitly.

**THE FULL ORDER, now in `BUILD_PLAN_2026-09-05.md`:** cleanup (done) → workspace membership → collaborative workspace → no customer data on our cloud → MFA → desktop (update channel first) → MAC screen → CoFFE upgrade. Two items sit outside it as decisions rather than builds: whether to change the drafting model so a sampling temperature applies (F25), and whether to wire the CoFFE shortest-route finding (B8.2).

## 05 Sep 2026 (night, register pass) — the register now reflects the night: F25/F26/F27 opened or closed, F24's exports closed, S12 part-done, S23-S27 added for the desktop.

**WHY.** Waqas: "close out what needs to get closed in the register then we move to the three urgent builds". `OPEN_ITEMS.md` 1,146 -> 1,222 lines, 87 -> 95 items.

**NEW / CLOSED IN F (AI engine quality).** **F25 — the temperature lever is inert on the shipped model** (open decision): `_modelAcceptsTemperature` omits temperature for Opus 4.7+ and `MODELS.reason` defaults to `claude-opus-4-8`, so lever 1 never reached the wire; confirmed by the exports' own `requestModel`. Half-fixed — the omission is now recorded per call — with the remaining half a DECISION for Waqas, because making it real means changing the drafting model and that is eval-gated. Recorded explicitly: this does NOT invalidate e1/e2/e3, which measured the configuration still shipping; only their LABEL was wrong, so F22/F23 improvements remain measurable against them. **F26 — the FHA prompt contradicted itself; CLOSED**, with a note that every pre-5-Sep draw was taken under the contradiction, so abstention rates from those runs measure a coin flip between two instructions rather than the model's judgement (E2's findings should be read that way). **F27 — the wall could mis-report its own failures; CLOSED**, 28 suites not 2, and eval/ now runs on a deploy.

**F24.** The exports half is CLOSED: e1/e2/e3 are out of the browser and on OneDrive with md5-verified copies and a README carrying the F25 caveat. Rule 26 satisfied. One question left for Waqas — whether a second copy belongs under `eval/runs/goldens/` (~8 MB in git) or OneDrive alone is enough.

**NEW IN S (desktop, from the 5 Sep audit).** S23 nothing authorizes a desktop user but a file on their own disk (tier defaults to `pro-plus`; Waqas's ruling recorded: fix it, notify him on sign-in, paywall the unlicensed — and the `notify-signin` machinery already exists, so a real sign-in delivers both as consequences); **S24 the update channel is unsigned — do this one first**, smaller than S23 and higher consequence; S25 Electron shell posture; S26 plaintext credentials on disk plus `app/` sixteen modules behind and missing `cloud_writer.js` (a data-integrity divergence, not tidiness); S27 the desktop repo has one commit and no remote.

**S12 is PARTLY DONE** (served-tree leak closed with an allowlist, stale proxy deleted and its test repointed at the real one, migration header corrected). Still open there: the missing DDL, `0007`, SSO.

## 05 Sep 2026 (night, cleanup batch 4) — the stale proxy deleted and its test repointed at the real one; three documents that lied corrected. NOT YET DEPLOYED.

**76.60 / core_modules 1.3 VERIFIED SERVED** after Waqas's deploy — and not by the pins alone. The served `ai_assistant.js` was fetched and read: the EMPTY-STRING severity order is gone, "WHEN THE INFORMATION IS THIN, JUDGE" is present, `_abstainForLane` is present, `_ABSTAIN_RULE` is still there for the other 23 lanes, and the served `core_modules.js` carries `temperatureApplied`. Pins: ai_assistant 76.60, core_modules 1.3, loader 8.51, helpers 2.85, bindings 1.36, severity_axes 1.3, fha_derive 1.1, eval_core 1.9, ai_skills 2.12.

**THE STALE PROXY IS GONE, AND THE TEST THAT BLOCKED ITS REMOVAL WAS THE REAL FINDING.** `safety-lab-proxy_worker_SEC.js` (14,679 B, 5 Jul fork, 184 lines behind the deployed worker, no sovereign ITAR routing) was flagged deletable by the cleanup inventory — but `regression_eula_terms` read it, so deleting it blind would have taken the wall red. Reading the test first showed something worse than a blocked deletion: the check compares the EULA's "US-sovereign hosted inference option is planned but not yet available" claim against the proxy's ITAR behaviour, **and it was comparing it against a file nobody ships**. It also SKIPPED SILENTLY when the file was absent (`if (proxy)`), so a check guarding a published legal claim could vanish and take the wall's green with it. Repointed at the sibling repo `safety-lab-proxy-deploy/worker.js`, made loud on absence in the same shape as `regression_notify_agents`, then the stale copy deleted. Verified first that the REAL proxy still satisfies the claim: it returns 503 `itar_unconfigured` when no sovereign backend is provisioned (two sites), so the EULA sentence stays accurate.

**MY WALL NOW SHOWS 2 FAILURES; WAQAS'S SHOWS 0 — MEASURED, NOT ASSUMED.** Both suites resolve `__dirname/../../safety-lab-proxy-deploy/worker.js`. On his Mac that is `/Users/waqasnafees/Desktop/safety-lab-proxy-deploy/worker.js`, which exists (23,071 B) because both repos sit on the Desktop. Through the Cowork mount the two are not siblings, so both miss. **274 suites, 2 real fails (both environment), 0 crashed here; 274 / 0 / 0 on his machine.**

**THREE DOCUMENTS THAT LIED, CORRECTED.** (1) `20260831_version_monotonic_guard.sql` said "NOT YET APPLIED" in its own header while `PERSISTENCE_INVENTORY.md:65` and a struck-through `OPEN_ITEMS` H-2 both record the sign-off, the MCP apply and the rolled-back probes. It matters because a customer rebuild follows these files and a migration that says do-not-apply gets skipped — and it propagated into the 5 Sep build plan, which repeated the header rather than checking the two records contradicting it. (2) `CUSTOMER_CLOUD_DEPLOYMENT.md` now opens with a DO-NOT-SEND banner naming its six defects, four fatal: `supabase db push` cannot rebuild the database (17 tables have no DDL anywhere); `window.AI_PROXY_BASE_URL` is read by nothing (the real name is `__SLAB_AI_ENDPOINT__`); it pointed at the proxy copy just deleted; and it omits the corpus, feedback and notification endpoints, so an install following it passes its own smoke test while still calling us on five paths. The banner also records that the migrations it tells a customer to apply currently carry the membership hole and the over-permissive `erase_project` grant. (3) `SECURITY_HARDENING_RUNBOOK.md` claimed enforced MFA — `index.html` ships `SL_MFA_REQUIRED = false` — so it now carries a correction saying so plainly, that it was switched off because it was broken rather than as policy, and that the claim must not be repeated to a customer, in a paper or on the trust page; its bare `npx wrangler deploy` (which skips the wall, the build and the smoke gate) is replaced with `./ship.sh`.

**S12 is now PARTLY DONE** in the register: served-tree leak closed, stale proxy gone, migration header corrected. Still open there: the missing DDL, `0007`, and SSO.

## 05 Sep 2026 (night, cleanup batch 3) — the FHA lanes stop contradicting themselves; a dropped sampling temperature is no longer silent. NOT YET DEPLOYED (76.60 / core_modules 1.3).

**(a) THE CONTRADICTION, REMOVED — SCOPED BY WAQAS.** `_ABSTAIN_RULE` ("any field you cannot ground must be returned as an EMPTY STRING … leaving a field empty is a GOOD outcome") and the shipped `fha.draft` skill body ("WHEN THE INFORMATION IS THIN, JUDGE - DO NOT ABSTAIN … an empty level silently drops the row from every downstream check") were in the SAME assembled request. Nothing in the code decided which won. Waqas: "the abstain instruction needs to be removed, we have judgement call flags now."

**The scope was put back to him with the evidence, and he chose FHA + SFHA only.** Only `fha.draft` and `sfha.draft` (2 of the 25 registered skills) carry the judgementCall/judgementNote contract — verified by counting `_BODIES_FHA_SHARED` users. The other 23 KEEP `_ABSTAIN_RULE`: stripping it there would trade a visible blank for a silent guess, which is the failure mode A10 exists for, and the E2 record says forced commitment landed near a coin flip.

**Five places changed** (`ai_assistant.js`, 76.59 → **76.60** in `ai_loader.js`): `_fhaSystemPrompt` no longer injects `_ABSTAIN_RULE` (covers AFHA and SFHA, both go through it); rule **2b** "Return severity as an EMPTY STRING" → judge, set the class, mark `judgementCall: true` with a `judgementNote` naming the missing fact, the assumption and what would overturn it; rule **2c** "A level you cannot ground stays EMPTY" → set by judgement and flag, "never left empty, because an empty level silently drops the row from every downstream check"; `_FEATURE_DIRECTIVE.fha` likewise (the "No Safety Effect is a finding about the aircraft, not a way of saying you do not know" sentence is KEPT); and `_anemBatch` now gates the append per lane (`_isFhaLane` / `_abstainForLane`) instead of adding it to every batch unconditionally.

**(b) THE SILENT TEMPERATURE DROP** (`core_modules.js` **1.2 → 1.3**). Anthropic deprecated temperature on Opus 4.7+ (a 400 otherwise), so `AiClient` omits it — correct, and it stays. But the default reason model is `claude-opus-4-8`, so the 5 Sep "every analytical drafter samples at temperature 0" lever was true of the client and FALSE OF THE WIRE, and e1/e2/e3 (~$15 and ~9 min each) were scored believing otherwise — the exported draws record `requestModel: claude-opus-4-8`, which is the confirmation. Omitting stays; doing it silently does not. `messages()` now computes `_tempAsked` / `_tempApplied` and both reach the per-call audit record on all three paths (success and both failures). Two now-false comments corrected: `_featureTemp` says what is ASKED for and states plainly that the lever has no wire effect on the shipped default; `_severityAnchorBlock`'s premise ("the model classifies at temperature > 0") is rewritten to the true reason it still earns its place. **Making the lever real means changing the drafting model, which is eval-gated against the reigning golden — a decision for Waqas, not a tweak.**

**NEW SUITE `tests/regression_fha_no_abstain_and_temp.test.js` (27 checks), mutation-proven seven ways:** put `_ABSTAIN_RULE` back in the FHA prompt → red; restore the EMPTY-STRING severity order → red; neuter the lane condition (`false ?`) → red; bypass the gated value at the call site → red; drop the temperature fields from the audit path → red; make `_modelAcceptsTemperature` lie about opus-4-8 → red. The third and fourth exist because the first draft asserted only that `_isFhaLane` and `_abstainForLane` APPEARED in the file, and the `false ?` mutation sailed through — the same lesson as the ship.sh anchor pins, a check satisfied by the wrong evidence is not a check.

**FOUR EXISTING SUITES WENT RED AND ALL FOUR WERE RIGHT TO.** `regression_eula_terms` asserted the FHA prompt "tells the model it may abstain" and pinned the old concatenation and the old directive — superseded, rewritten to the new doctrine with the reasoning in the file, and the defect they were written for (the model reaching for Major because Major looks considered) is still guarded, now via "Do NOT reach for the benign end of the scale". `regression_spec_reachability` pinned the literal `+ '\n\n' + _ABSTAIN_RULE` — updated to `+ _abstainForLane`; its invariant (both paths go through ONE assembler) is untouched. `regression_anem_unreadable_reason` and `regression_fha_derive` pinned `ai_assistant 76.59` EXACTLY, so an unrelated bump broke checks about ANEM replies and derive load order — both converted to FLOORS via `tests/lib/pinfloor.js` (WORKING_RULES rule 12: pins as floors, never literals; a check that cries wolf is a check people start editing without reading).

**WALL: 274 suites, 1 real fail, 0 crashed** — the environment-only `regression_notify_agents` [P4] proxy-sibling one.

**NOT DEPLOYED.** Ship command is Waqas's. After deploying, verify the SERVED pins: ai_assistant 76.60, core_modules 1.3, loader 8.51, helpers 2.85, bindings 1.36, severity_axes 1.3, fha_derive 1.1, eval_core 1.9.
...
**BASELINE FIRST, AND A PROBE LESSON.** The wall was run directly (NOT via `ship.sh` — deploys are Waqas's) before touching anything: **271 suites, 1 real fail, 0 crashed**. The one failure is `regression_notify_agents.test.js` [P4], which requires `safety-lab-proxy-deploy` checked out *beside* `safety-lab-deploy`. On Waqas's Mac it is (both on the Desktop); through the Cowork mount the two are not siblings, so the relative path misses. **Environment, not defect — the wall is green on his machine.** Rule 14: suspect the probe. Baseline for this session is therefore "exactly that one failure, nothing else".

**REMOVED (8 files, 579 KB, all proven to have no referencing file first).** `deploy.sh` (3,267 B — executable, copied from a Claude outputs folder, could ship stale files; `ship.sh`'s own header said never use it, and that header is now updated to record the deletion); `qa_boot_pp.js`, `qa_hf_kb.js`, `qa_stpa_kb.js` (15,551 B — Playwright QA drivers superseded by `tools/smoke/smoke_gate.js`; only mention left is historical, in `docs/BACKLOG-2026-07-25.md`); `event_trees.v1.0.bak.js` (17,917 B); `index.html.bak.collapsible` (336,806 B); `safety_lab.css.bak.collapsible` (205,221 B); `supabase/functions/notify-invite/DEPLOYED.md` (335 B, self-superseding); and the empty `site/rams/`. `.wrangler/` (miniflare cache, 3 files) removed from the git index and added to `.gitignore`.

**NOT REMOVED, AND WHY.** `safety-lab-proxy_worker_SEC.js` — the cleanup inventory marks it delete-worthy (a 5 Jul fork of the real proxy, 184 lines behind, no sovereign ITAR routing), **but `tests/regression_eula_terms.test.js` reads it**. Deleting it would have taken the wall red. It needs the test repointed at `safety-lab-proxy-deploy/worker.js` first, plus the four documents that cite it corrected. Left in place; this is the "needs-a-human" the inventory flagged, now with the reason confirmed by measurement rather than reading.

**AFTER: 271 suites, 1 real fail, 0 crashed — identical to baseline.** Nothing broke.

**SCRATCH CLEARED TOO (Waqas: "anything that is safe to clear and does not break anything you can go ahead and clear").** `_bak` was READ FIRST per rule 26 before anything was touched: 89 files, all source snapshots (.js/.html), no measurement data — the `20260831-dataloss/` folder holds a copy of `eval_core.js`, i.e. the code under investigation, not the lost draws. All eight folders were confirmed untracked (`git ls-files` = 0 each) and referenced only by documentation and `.gitignore`, never by code. **Archived first** to `OneDrive-SafetyLabAero/Safety Lab Aero - repo archive/safety-lab-deploy_scratch_2026-09-05.tar.gz` (34 MB, 519 entries, archive verified readable and its top-level members listed) and only then removed: `_bak` 34 MB, `_to_delete` 19 MB, `backups-2026-07-11` 15 MB, `-12` 4.7 MB, `-15` 3.4 MB, `-14` 3.0 MB, `_claude_stage` 740 KB, `Claude outputs` 532 KB. **Working tree 176 MB → 101 MB.** The `.gitignore` entries were left in place so the folders cannot re-enter git if they are recreated. Wall re-run after: **271 suites, 1 real fail (the environment-only proxy-sibling one), 0 crashed** — unchanged.

## 05 Sep 2026 (night) — the served tree was publishing a full copy of ai_assistant.js; build.sh now has an allowlist. NOT YET DEPLOYED.

**THE LEAK.** `build.sh` step 2 copied every non-`.js` top-level file out of `site/` into `dist/` with no filter (`find "$SRC" -maxdepth 1 -type f ! -name '*.js'`). Two hidden FUSE leftovers were sitting in `site/` and were therefore published: **`.fuse_hidden0000001500000001` (919,444 bytes — a complete copy of `ai_assistant.js`: every prompt, every skill body, "Confidential / Patent pending" comments intact)** and `.fuse_hidden0000003900000001` (354,851 bytes — a copy of `index.html`). Because neither ends in `.html`, both also bypassed the comment strip at step 3b. Verified present in `dist/` (plus a macOS collision copy, `… 3`). Also published: `site/MS_SSO_SETUP.md` and `site/gt_thread.js.v2bak`. Found by the cleanup inventory Waqas ordered ("clean up first > build on the three very high priority tasks > then the rest of the gamut"); he ruled "the urgent needs to get fixed".

**THE FIX (not deployed).** `build.sh` step 2 is now an explicit ALLOWLIST — `html css png jpg jpeg webp gif svg ico xml txt json woff woff2 map`, and `! -name '.*'` so no dotfile can ever ship. A denylist would have to predict the next kind of junk. New step 2b prints every top-level file in `site/` that was NOT published, so a new asset type must be added deliberately rather than vanishing silently, and new junk is visible on every build. Proven before/after with the bare `find` expressions: all 22 real assets still publish; exactly the four offenders drop out. `bash -n` clean. The four files were MOVED, not deleted (rule 26), to `_to_delete/2026-09-05-served-tree-leak/`; `MS_SSO_SETUP.md` and `gt_thread.js.v2bak` were git-tracked, so they show as deletions. `build.sh` backed up to `_bak/build.sh.pre-allowlist-2026-09-05`.

**STILL TO DO ON THIS.** Waqas deploys — until then the leaked copies are still live, because `dist/` is only rebuilt by `build.sh`. Worth checking the Cloudflare request logs for hits on `/.fuse_hidden*`, `/MS_SSO_SETUP.md` and `/gt_thread.js.v2bak` to know whether anyone actually fetched them. The subdirectories copied wholesale at step 3 (`kb rams assets downloads vendor`) were checked and are clean.

**ALSO CORRECTED.** `BUILD_PLAN_2026-09-05.md` said the version-monotonic guard migration was unapplied. It IS applied — its own file header is the false document (`OPEN_ITEMS.md:966`, `PERSISTENCE_INVENTORY.md:65` record the sign-off, apply and rolled-back probes). The plan repeated the header rather than checking it. Corrected in place, and noted there as an example of the defect class the cleanup exists to remove.

## 05 Sep 2026 (late) — end-to-end product and code survey before any build; sixteen decisions agreed; the RLS ruling revised on evidence. Nothing built.

**WHY.** Waqas: "I do not want us repeating what we have done on previous sessions, I do not want screwing up what is in good health, so before you touch anything I want you to have an understanding of the product end to end along with the code" and "I want you reading all the files attached in full". Read in full: HANDOFF.md (5,041 lines, 133 entries) digested per-entry with a de-duplicated doctrine and rejected-ideas index; ARP4761A (692 pp) and ARP4754B (172 pp) page by page; six read-only code surveys — shell/state, AI path, FHA/severity, thread/MAC/CoFFE/trees, auth/cloud/security, tests/eval/ship. Repo untouched throughout; commit `dcf2a1d` at start and end.

**FINDINGS THAT CHANGED THE PLAN.** (a) The FHA prompt tells the model to abstain (`ai_assistant.js:2675-2680`, `:12739`, `:1568`) and to JUDGE - DO NOT ABSTAIN (`ai_skills.js:35`) in the SAME assembled request — Waqas: "the abstain instruction needs to be removed, we have judgement call flags now". (b) Temperature 0 likely never reached the model: `MODELS.reason` = `claude-opus-4-8` (`:702`) and `_modelAcceptsTemperature` strips temperature for Opus >= 4.7 (`core_modules.js:262-270`) — the 5 Sep lever 1 had no wire effect, so e1/e2/e3 measured a different configuration than believed. Waqas: "needs to get fixed". (c) The escape rule runs FIRST at accept and its `nse` branch forces all three axes to the bottom and RETURNS (`fha_derive.js:360-364`), skipping MAC and HF derivation and filing no pending-validation assumption — the mechanism behind the thirteen airborne rows. Waqas: the outcome stands where true, but it "should show in the comments" so the user corrects the posture. (d) S1 confirmed at `0001_rls_baseline.sql:108-109`, and the reason it hid three reviews: `regression_h8_rls_disk_sync.test.js:76-84` filters for the seven helper names and this policy contains none. (e) TWO WORSE THAN THE AUDIT SAID: `erase_project` is granted to `authenticated` on disk (`0005:41,43`) while live is service_role — a rebuild from the repo would be a security regression; and `lock_seal.js:91` has NO SLEnv fallback unlike `stale_watch.js:53` / `lane_trees.js:75`, so an aircraft FHA's seal is a literal constant and INV-19 reports green over a hand-edited FHA. (f) F23's blocker: a Task Analysis row has no field pointing at a failure condition. (g) `regression_e2_commitment` and `regression_invitations` can fail through a green wall; `eval/regression_ai_repeatability.test.js` has never run on a deploy.

**SIXTEEN DECISIONS AGREED — full trail with reasoning in `DECISIONS_2026-09-05_scope.md` (repo root, committed).** Membership: creator is owner automatically, nobody enters without an invite, only the owner invites unless they grant admin. Only the owner transfers ownership; admins cannot evict the owner. MFA was off because it was BROKEN — fix and enable. Security headers get the proper fix (worker owns the SPA fallback, real 404s). No customers exist, so the seal check may fail loudly. Escape rule shows its reasoning in the comments. Occupant axis is judged wherever occupants are aboard — "option C off" was Aeolus-specific, not general. Crew tasks get explicit links to the conditions they answer and to their assignee. Fix both non-canonical-FAIL suites AND add the enforcing check. The `erase_project` drift rides with the membership migration. Widen `ship.sh` to run `eval/*.test.js`. The e1/e2/e3 exports go to Waqas's OneDrive (rule 26 — they exist only in a browser today).

**THE RULING THAT WAS REVISED, AND WHY.** Waqas first ruled "I do not want helpers, I want it by policy" then "clean it up the helpers and write the whole thing as a policy". That cannot be done: the seven `private` helpers are SECURITY DEFINER precisely so a policy can read `workspace_members` without recursing into that table's own RLS — `0008` says so in its own comment. Put back to him as "can this be fixed properly THROUGH helpers?" — answer yes, and it is the smaller change. AGREED FIX: keep the helpers; add ONE new helper reading `workspaces.owner_id` (the creator is not yet a member at creation); correct `ws_members_self_join` to self-insert only into a workspace the caller owns; add WITH CHECK to `ws_members_admin_update`; guard deletion of an owner row; a dedicated owner-only transfer RPC; drop the two hardcoded emails in `is_safety_lab_admin()`; correct the `erase_project` grant in the same migration; widen `regression_h8_rls_disk_sync` so a policy naming no helper cannot hide; publish the permission model as a one-page table backed by a test. Jama comparison (researched): Jama has NO self-join at all — admin-granted only, org and project admins, users or groups, inheriting org > project > folder > item, highest permission wins; Safety Lab's creator-owner + invite model is deliberately friendlier and closer to SharePoint/Google Workspace. The idea worth taking from Jama is permission INHERITANCE, which Safety Lab does not have (access is flat per workspace) — settle it before the customer-hosted build.

**ORDER (Waqas's call, argued the other way and not accepted).** "we fix customer data on our cloud and collaborative sync first then MFA". So: (1) NO CUSTOMER DATA ON OUR CLOUD AT ALL — not just ITAR — it stays on customer servers, plus step-by-step customer instructions; (2) collaborative sync exactly like Microsoft SharePoint, no "project state has changed" prompts ("that is a nuissance"), every user's state updates live, workspace locking already exists to stop two people editing one document; (3) MFA; (4) the MAC screen, which is "messed up" and needs to be markedly more intuitive. Waqas: "100% no half ass fixes this time." RECORDED CONSEQUENCE, stated to him and accepted: the cross-tenant membership hole stays OPEN while (1) and (2) are built. It is a self-contained migration touching no app code and can be prepared in parallel.

**ALSO.** Waqas lifted the standing "never read, stage or request access" rule on his ITAR build — "you can read the ITAR build" — after being told that anything read lands in the cloud session regardless of how it is read. The folder is NOT at the Desktop top level and its location is still to be confirmed. The desktop application source is `~/Desktop/safety-lab-desktop` (Electron: `main.js`, `license.js`, preload/settings/signin, notarize, publish scripts); Waqas does not know what authorizes it, so it is to be audited — until then every "MFA is enforced" and "sessions expire" claim is web-only.

## 05 Sep 2026 (night) — security code audit before the Data Security paper: six critical defects, no audit writer, ITAR fences that do not hold. Nothing built yet.

**WHY.** Waqas: "look up data security white paper, update what needs to be updated as suited to Enterprise level modules, identify what gaps there are, and I want how the AI is managing any data" → "we build then we write the papers" → target "Microsoft Office / Jama Connect level" (Part 2 audits and Part 3 government tier ruled OUT; engineering only) → "go audit the code to make sure you understand what is actually built". Three read-only passes over a snapshot of site/, worker.js, migrations, edge functions, the proxy repo and the Teams bot, plus live pg_policies / trigger / served-header checks. Full record with file:line evidence: **`SECURITY_AUDIT_2026-09-05.md`** (repo root, committed).

**CRITICAL (fix before any paper claim).** (1) `ws_members_self_join` RLS on workspace_members is `WITH CHECK (auth.uid() = user_id)` and nothing else, no trigger — any signed-in user who learns a workspace id can add themselves as owner (verified live). (2) MFA master switch is OFF (`index.html:4007 SL_MFA_REQUIRED=false`); enforcement is client-side, fail-open, desktop bypass — runbook and SL-WP-0003 say enforced. (3) ITAR fences: `crdt_sync.js:59` / `presence.js:38` read `window.projectConfig` which is never assigned → CRDT and presence run on ITAR projects; manual Save-to-cloud and Create Revision upload ITAR projects incl. full source-document text; notify sender has no ITAR gate; controlled-document guard runs only on the ANEM path, classic lanes include controlled docs; "ITAR cloud" backend choice does not set the ITAR header. (4) `/app/index.html` and any unknown `/app/<path>` are served with NO CSP/HSTS/XFO (SPA fallback bypasses serveHtml; verified live). (5) `lock_seal.js:91` reads stores via eval, blocked by the production CSP → FHA/requirements/CCA/items/FMEA are not actually sealed. (6) `trust.html:164` says customer content may be used to train models — contradicts the EULA, the paper and the no-training test.

**HIGH.** audit_log / workspace_audit / ai_usage have NO writer (audit_log 0 rows) — the only AI-call log is `projectConfig.aiAuditLog` in the project file; licence token, BYO API keys, Jama username+password, bridge token in plaintext localStorage; erasure: erase_project is service-role only, certificate shown once and never persisted, local copies never cleared, ai_org_cache (per-user, owner-only RLS — not org-wide) survives erasure; notify-* edge functions accept any bearer ≥16 chars; proxy rate limit fails open and writes the raw token into KV keys; migration drift (users/workspaces/projects/audit_log/approved_tenants/ai_org_cache/destruction_certificates/erase bodies exist only in production; version-monotonic guard not applied); stale `.fuse_hidden*` builds and MS_SSO_SETUP.md shipped under site/; stale proxy copy `safety-lab-proxy_worker_SEC.js` in this repo (the real one is safety-lab-proxy-deploy — it DOES have the notify routes and the Bedrock/Azure sovereign router, unprovisioned).

**WHAT HOLDS.** RLS on every content table via private-schema helpers; server-side invitation redemption; Postgres sign-off chain + verify RPC; document-loss guard + version pruning; archive; idle sign-out (web); recovery flow; content-stateless proxy that refuses ITAR to public Anthropic (503 until sovereign secrets set); CSP on the main document; security.txt; journal / locks / reopen scope / cascade / replay — real but browser-side over the editable project file; evidence packages (no raw docs or AI output, but verbatim citation quotes).

**THE AI DATA PATH, AS BUILT** (for SL-WP-0003 Rev 3.0): a prompt carries the WHOLE text of every uploaded source document (or spec-index chapters), all tables, image captions, under the file name; golden-thread rows; the project's own rows as exemplars; up to 24 lines of this browser's AI-memory diffs (skipped on ITAR); attached diagrams as base64; the first 300 chars of the user message also go to the corpus search endpoint. No PII/secret scrubbing. Stored after a call: row provenance + comments, `aiAuditLog`, `aiProvenance`, `aiDrafts`, `aiEvidenceRecords` (includes result text), assumptions ledger with citation quotes, AiMemory IndexedDB, local draft cache (response text), remote `ai_org_cache`, eval exports. Voyage embeddings exist in transport but only the connection test calls them.

**REGISTERED (Waqas: "log AI consistency tasks as open items, log these as open items").** OPEN_ITEMS.md now carries **F22** ("felt yet" as a property of the function — lever A), **F23** (feed the MAC and the crew tasks — lever B), **F24** (three fresh draws after F22+F23, read the residual; option C is off — the drafter says "no passengers" consistently on Aeolus), and a new **section S · Security and data residency**: S1–S6 the critical defects, S7–S12 the high list, S13–S18 the Enterprise builds (unified activity log; one classification label every channel obeys; retention + legal hold + complete destruction certificate; workspace admin controls; AI data receipt; customer-managed keys last), S19–S21 data residency, S22 the paper and pages.

**DATA RESIDENCY (Waqas: "can the data live in the customer's cloud/server instead of browser memory?").** Three options stated, none started: **S19 thin-browser mode** (~1 week — nothing persists in the browser between sessions; saves go straight to the server; closes the plaintext-credential finding and half the erasure gap by construction); **S20 customer-hosted backend** (~2–3 weeks — database, sign-in, real-time and the AI gateway in the customer's own Supabase org / AWS account; we host only the static app and keep no copy; needs the backend URL/key as an installation setting — today hard-coded in `safety_lab.js:3088` and the CSP — packaged migrations (depends on S12 capturing the production-only DDL), packaged edge functions and proxy, a licence check that does not phone home, their Entra SSO, their model endpoint); **S21 fully self-hosted / air-gapped** (S20 + packaging). Recommended: S19 now, S20 as the Enterprise deliverable, S21 falls out of S20. The six criticals come first regardless — a customer-hosted backend inherits every one of them otherwise.

**NEXT (nothing started; each build stated back first).** S1–S6, then S7–S12, then S13–S18, S19–S21 as Waqas orders them, then S22 (the paper is written after the builds: "we build then we write the papers"). Audit tarballs removed from Downloads.

## 05 Sep 2026 (evening) — three identical-input draws on the lever build: function worst case clears 90%; row severity 67–72%; the phase split and the "felt yet" answer are the residual.

**SCORER 1.9 (a24b978, not yet deployed) — THE BAR IS CLASS PER CONDITION PER PHASE.** Waqas, on the draw report: "same class different effect we just ruled doesnt need to be on the same row." Row pairing and phase-split matching were punishing presentation. `perPhaseClassAgreement` ≥ 0.90 is the judged number now (rows expanded to phases; "All phases" = profile; a phase named twice takes the worst class; abstentions never count as agreement; byPhase + worstConditions reported); `severityAgreement` (strict rows) and `phaseSplitAgreement` are informational. Re-scored e1/e2/e3 under it: **0.720 / 0.656 / 0.635** (1,174–1,177 cells a pair; 131–168 two-class jumps; 18–28 abstain cells). Function worst case unchanged at 0.91 / 0.96 / 0.96. Exports still to be filed in the repo and scored by the CLI.

**THE DRAWS (e1/e2/e3, fha_derive 1.1 + 76.59 served; fresh reload each, FHA cleared, cache bypassed; 199 / 200 / 197 rows; 9.4 / 8.6 / 9.4 min; ~$15).** Exports are in the draws tab's IndexedDB (`SLDB.get('slab.draw5.e1'…'e3')`, 2.5 MB each — localStorage refused them, which cost draw e1 its in-memory record; it was rebuilt from the 199 restored rows after reload) and in Waqas's Downloads via `runRepeatabilityExport()`. Not yet filed under eval/runs/goldens/. Strict scoring (eval_core 1.8), pairs e1v2 / e2v3 / e1v3: **functionWorstCaseAgreement 0.909 / 0.955 / 0.955 — PASS at 0.90** (was 0.82); fhaSignatureMatchRate 0.955–0.965; strictPairRate 0.83–0.86; **severityAgreement 0.721 / 0.669 / 0.673 — FAIL** (was 0.63); severeJumpRate 0.06–0.08; **phaseSplitAgreement 0.449 / 0.383 / 0.318 — FAIL**; same row count on 73 / 65 / 59 of 107 conditions. Per (condition, phase) cell — the truest read, ~1,175 cells a pair — class agreement **71% / 66% / 63%**.

**WHERE THE DISAGREEMENT IS.** About a third of the disagreeing cells (109–138 a pair) are the escape ANSWER flipping — one draw says not realised + escapable (NSE by rule), the other says realised (Hazardous or worse); ground phases (Standing / Taxi / Rejected Takeoff) worst. The rule is applied the same way every time; its input is a judgement the model does not repeat. The other two-thirds are judged axis levels — aircraft and crew roughly equal (~220–280 cells), occupants fewer (~110–150). On run 3's inputs the derivation reached only 10 partial losses (MAC rules on 9 of 22 functions) and no crew rows (the 6 tasks carry no phases, times or condition links), and the MAC-derived rows never paired exactly (their phase groups moved), so the derivation's own stability is unmeasured. A draw-1 lesson already fixed before the three: the drafter's own "escape: none" must count as no escape (13 airborne total-loss rows had become NSE) — fha_derive 1.1, 329906f.

**PROPOSED TO WAQAS, NOT BUILT.** (A) "Felt yet" as a property of the FUNCTION: a per-function "demanded in these phases" list, drafted once and fixed with the functions and FCIM — not demanded in a phase → not realised there, by rule; removes most of source 1. (B) Feed the derivations: MAC rules for all 22 functions; the crew tasks given phases, response times and the conditions they answer. (C) Occupant axis mapped from the aircraft axis (doctrine question for him).

**ALSO THIS EVENING.** FHA tables (helpers 2.85, 1859655): FC ID column is the id alone — the group finding opens the Comments cell instead; Sub-Function a bit wider, Effects and Comments 480px / 34%. Grouping is effect-aware (f888900): two rows of a condition with the same class are a legitimate split when their effects differ; "should be one row" only when class AND effects match; a phase carrying two different effects is still assessed twice. Waqas: "two rows with same severities are fine as long as effects are different … this is just an example dont treat it as absolute." One ship refused on `regression_fha_phase_rows` failing to START (it extracts `_fhaGroupRows` alone and did not know `_fhaEffectKey`) — c8b7ded; lesson: read the WALL GREEN line of a dry run, not the fail count. Draw 1 (d1) ran before the 1.1 fix and was discarded.

## 05 Sep 2026 (midday) — the consistency levers built: temperature 0, escapes on the mission profile, MAC/HF-derived levels, a phase-split metric. Draws next.

**THE FOUR LEVERS WAQAS APPROVED ("I dont like 5 but the rest look good") ARE ALL IN.** Lever 1 shipped in the morning (9927325, 76.58): every analytical drafter samples at temperature 0 — the batch path was labelled `chat.edit` and ran at 0.3. Lever 4 (85dcd64, eval_core 1.8): `phaseSplitAgreement` ≥ 0.90 with a per-condition flip report saying what moved, the split or the class. Levers 2 and 3 are this entry (new module `site/fha_derive.js` v1.0, 76.59, loader 8.51, helpers 2.84, bindings 1.36, severity_axes 1.3; suite `tests/regression_fha_derive.test.js`, 51 checks; wall 271 green). Nothing has been deployed since 76.58 — the ship command is with Waqas, and NO DRAWS until 76.59 is served ("no draws till all the fixes are in").

**LEVER 3 — ESCAPES ON THE MISSION PROFILE.** Every phase row now carries `escape`: how the flight gets out of a condition whose effect has not yet been felt (Standing / Taxi: stop on the ground; Takeoff: reject the take-off before V1; Initial Climb → Descent: continue to a landing; Approach: go-around; Landing: none; Rejected Takeoff: stop on the runway; Go-around: continue to a landing). Seeded on new projects; an older project reads the same defaults by phase name; editable in a new Escape column on Define → Flight Phases (type `none` for no way out). Both FHA prompts (the unified batch's system extra and the classic rule 4) carry ESCAPES BY PHASE and ask three structured answers per row — `realized`, `escape`, `escapeDefeated` — and accept applies the 4 Sep ruling from them, not from prose: not realised + escapable + not defeated → No Safety Effect (all three axes none, marked "by rule"); not realised + no escape or the escape defeated → the end effect stands; a row mixing Landing with an escapable phase carries the end effect and is told to split; an escape the engineer has not stated does not fire. The drafted end-effect levels of an NSE row are kept in the comments, not lost.

**LEVER 2 (F21) — LEVELS DERIVED, NOT JUDGED.** Aircraft axis from the MAC rule for the function (`projectConfig.macModels`, keyed on the row's sub-function — for an SFHA, the aircraft sub-functions the system function traces to, one shared helper `_macSubIdsFor`): a PARTIAL loss still inside the rule is graded by the authority left against the floor — on the floor → large; one spare → significant; two or more → slight (how many members the loss removes is read from the wording, default one; a "partial" that removes more than the rule allows is outside the MAC). A phase-specific rule wins in its phase; a row whose phases grade differently takes the worst and is told to split. A TOTAL loss is outside the MAC — the level is the loss itself, judged, with the escape rule on top. A malfunction is not a configuration question; the MAC does not govern it. No rule for the function → the drafter's level stands and an assumption "assumed pending the MAC rule for SF-nnn" lands in the register. Crew axis from the Task Analysis (`projectConfig.hf.tasks.rows`): the tasks tied to the condition (named by condition id or function id in the task text, or reached through an error-analysis row's `fcIds` → `asmId`) are read on the row's phases through the HF lane's own `taskOccupancy` — up to 60% slight, 60–80% significant, above 80% large; a response that cannot be completed in the time available is large and flagged. No timed task → "assumed pending the human-factors task analysis" in the register. The occupant axis stays the drafter's. The drafter is TOLD the derived levels per turn (DERIVED LEVELS clause, `extraOf` on the FHA chunk; the classic path's user message carries it for the batch's FCIM conditions) and asked to write the sentences and judge only the rest; accept enforces the same derivation on the drafted levels BEFORE the joint top step, records what it decided (and what the drafter proposed) in the comments, and the Effects cell shows "from MAC" / "from HF" / "by rule" beside the chip. Order at accept: escape rule → MAC → HF → joint top step → worst-axis class.

**WHAT THIS DOES NOT DO.** It does not make the occupant axis or a malfunction's aircraft level consistent — those remain judgement, and the 90% bar will show how much of the disagreement they carry. On a project with no MAC and no Task Analysis (run 3's inputs have MAC rules for 9 of 22 functions and no crew tasks) the aircraft axis is derived only for partial losses under those 9 rules; everything else lands as assumptions, exactly as Waqas said it should ("they will have to get validated later as those analyses are populated").

**NEXT, IN ORDER.** (1) Waqas ships; verify the SERVED pins (76.59 / loader 8.51 / helpers 2.84 / bindings 1.36 / severity_axes 1.3 / fha_derive 1.0 / eval_core 1.8). (2) Three fresh FHA draws on identical inputs — run 3's 22 functions / 43 FCIM / 107 conditions, a FRESH session each with an empty FHA (Waqas: "by scrap old rows I meant start a fresh session"), the draws tab detached from the cloud — scored strictly with eval_core 1.8 (severity, function worst case, phase split, flips), judged against 0.90. (3) Read the flips: if the split still wobbles, the escape answers are where to look; if the class wobbles on paired rows, the axis notes say whether it was a derived axis (a data question) or a judged one.

## 05 Sep 2026 (early) — consistency measured honestly; the FHA row doctrine enforced in code; severity's real fix named.

**THE CONSISTENCY NUMBER, CORRECTED (eval_core 1.7).** Three-run scoring across runs 1/2/3 read severity agreement 0.40–0.55 — and a quarter of that was the scorer: its topic lexicon paired "loss of thrust production" with "loss of ground reverse thrust" and compared their classes. Severity metrics now run on STRICT pairs (same condition — shared id, or same loss form + condition wording — on the same phase group); bar 0.90 (Waqas: "the numbers need to be over 90 percent"); NEW `functionWorstCaseAgreement` ≥ 0.90 (worst class per sub-function — the headline an engineer reads; row-count differences are abstraction level and do not move it); `strictPairRate` informational. Goldens promoted under the old 0.50 bar fail the new one and are read as such (regression_ai_repeatability updated, tightening table in EXPORT_RUN). Content overlap across the three runs is high (functions 1.0, FCIM 0.87, ZSA 0.93, HFA 0.97); the weak lanes are severity (0.46 old scorer / 0.62 strict on runs 2 vs 3) and Task Analysis (0.45).

**RUN 3 (F16 batch live) AND THE FHA DEFECT IT EXPOSED.** Run 3 on 76.54: 22 fn → 43 FCIM → 107 FHA → 5 systems (the model named 10, added 5 — fixed: `add_function` creates the owner system it names, arch.systems v2) → 9 MAC → **1,025 interdependence proposals, 0 failures** (run 2: 0 — F16a proven live) → CoFFE 424 verdicts in 45 min (101 calls, 44 failed, no reasons yet — F20) → 9 rules → 18 compiled tree pages (idp-uncovered now only where a contributing system is truly outside the rule; 102 "MF&MS — seeded" skeleton pages still created beside them — F20) → PRA 7 / ZSA 12 / CMA 14 of 21 (F13) → HF lanes. FMEA declined WITH a reason this time (F16d) and is out of the thread ("leave FMEAs out for now"). Then Waqas, on the FHA table: "it doesnt break out the rows properly generally assess all phases, or when it breaks them out it only assesses one row per failure condition" — run 3 on fha.draft v7 (the 3 Sep row doctrine, in the skill body) gave 0 of 107 conditions a second row, 58 rows "All phases", 2 No Safety Effect. **Audit found five later instructions arguing with the rule** — system prompt rule 4 "phases where the condition is most relevant", the batch extra "One row per listed condition.", the per-turn "Emit a row for EVERY failure condition", the classic path's "(a second row … only where …)", the picker's "One FHA row per selected condition." — and coverage counting a condition done when it had any row. All five replaced with the ruling (any number of rows; every condition across every phase; abortable-and-unrealised = No Safety Effect; unrealised-and-inescapable = the end effect; "All phases" only when identical everywhere); phase-coverage check with one re-ask and a panel line for what is still wrong. **Rulings 5 Sep: "one phase of flight can only have one effect and severity per failure condition"** — a phase on two rows of one condition is assessed twice (engine check, re-ask, table badge "phase assessed twice"); **"scrap old rows"** was MISREAD for an hour as a product rule (accept replaces the AI's earlier rows of a condition) and built; Waqas: "I meant start a fresh session not scrap old rows when something is accepted" — reverted the same night, accept semantics are the 3 Sep upsert (same condition + phase set updates in place, hand-edited protected, a different phase set is a new row). The rule is a MEASUREMENT rule: every draw starts from an empty FHA in a fresh session. The "phase group × N · worst X" pill is gone (standard practice earns no badge). NOT deployed as of this entry: 7d7c127 (76.57, loader 8.49, helpers 2.83, eval_core 1.7 already live).

**IDENTICAL-INPUT DRAWS (same 22 functions + 43 FCIM, FHA redrawn from empty, cache bypassed).** Draw 2 and 3 on the row-fix build: 200 and 180 rows, 78/70 conditions split by phase, "All phases" 1/0, No Safety Effect rows 47/29, every phase covered. Strict scoring draw 2 vs 3: rows pair 80%, **same severity 63%, worst class per function 82% (18 of 22)**, 6% two-class jumps. Where it sits: the phase split itself wobbles (only 74 of 107 conditions got the same row count); one-step swaps dominate (Hazardous↔Major 20, Negligible↔Minor 14); aircraft/crew axis levels agree only 63%/61%. **Waqas's direction for the fix: severity levels are judgement calls no engineer gives twice the same; lean on the MAC for the aircraft axis (how much configuration authority remains → slight/significant/large; outside MAC → the loss) and on Human Factors crew workload (task-analysis occupancy against the 60%/80% lines) for the crew axis, with the AI's level a placeholder assumption "pending MAC / pending HF" until those analyses populate.** Two proposals rejected and not to be re-raised: a per-function worst-case anchor ("takes us back to the faux pas from august 2nd") and fixed phase groups from the mission profile (loss of braking: Standing NSE, Taxi low-speed collision Maj/Haz, airborne forward-looking CAT, Landing immediate CAT — groups come from each condition's effects, never a template). He wants **three fresh draws on identical inputs AFTER the derived-levels build ships**, judged against 90. The draws project was detached and closed; nothing from it is saved.

**PROCESS.** A compaction lost the 3 Sep row ruling and I re-proposed a version of it; the commit log (09d83f1) and the skill body had it all along — read both before proposing FHA structure again. The deploy that shipped the F16 batch missed the last commit by minutes: verify the SERVED pins after every ship, not the terminal.

## 04 Sep 2026 (evening) — the full thread built and run (run 2); loss-forms settled; the demo "AI backend not ready" race closed; Radia ROM issued.

**THE FULL THREAD (F15) IS BUILT AND WAS EXERCISED END TO END ON RUN 2.** New lanes, all in `site/ai_assistant.js` with bodies in `site/ai_skills.js` (27 skills): `arch.systems` (systems + system functions with traceIds, every redundant copy kept countable), `arch.items` (one configuration item per redundant copy under its system — added mid-run when the model collapsed redundancy into one function), `mac.draft` (`add_mac {subId, phase, clauses:[{min, of:[configuration item ids]}], sddRef, rationale}`; members resolve by id, internalId or name; the phase must be a mission-profile box; a signed rule is never overwritten), `coffe.draft` (direct lane, residue only, verdicts marked AI-proposed with the model as signer; the panel shows "✨ AI-proposed"). Harness THREAD is now decompose → fcim → fha → systems → items → mac → sfcim/sfha per system → resources → interdep → coffe → trees (`SLLaneTrees.compileAll()`) → fmea → pra → zsa → cma → hf. Run 2 project "Aeolus HL-1 · Golden thread · run 2 · 4 Sep" compiled 19 MAC rules into 33 tree pages with 42 findings; its export is in Waqas's Downloads and NOT yet filed under `eval/runs/goldens/`. What run 2 taught (all on the register): the interdependence sweep made 25 calls and landed 0 proposals — undiagnosed; `lane_trees` does not treat a configuration item that is a MAC member as covering its owner system's functions (`idp-uncovered` on every page); the MAC drafter proposes no arbitration scheme (`lane-mal` "No arbitration scheme declared"); FMEA declined without a reason; CoFFE residue was 1,389 cases and was cut short. Tests: regression_thread_systems, _mac, _coffe, _asm_one_table, _license_ready; wall 264 green.

**MAC RULINGS (Waqas, 4 Sep), binding.** A MAC rule reads "at least N of these configuration items must be available" — items, not system functions ("if you have 2 completely redundant flight control systems, you need at least 1 available"); symmetric minima are per-side clauses ("2 lost on one side you are done for"); the idea is how much control authority must remain for continued safe flight and landing. Order of the thread: aircraft functions → MAC for those functions → FCIM → FHA → resources/interdependence → CoFFE → fault trees; system functions are allocated through interdependence ("chicken or the egg"). Total loss = loss outside the MAC; partial loss = loss within it. **Condition wording flip-flopped in one day and settled at fcim.draft v4 (76.52):** conditions read "Total loss of X" / "Partial loss of X" — never "MAC" in a condition — and the MAC definition and rule go in the FHA row's comment (`_macCommentFor`: "MAC — total loss: … Rule: at least 1 of [FCC A, FCC B] (SDD §4.2)" or "Rule: not yet defined for this function"). Aircraft-level FHA never counts copies; the system level parses the MAC detail. Do not reopen the wording.

**"AI BACKEND NOT READY" IN THE MIDDLE OF A DEMO — closed (auth_gate 62.67, 76.53, loader 8.45; 33eb74a; live-verified on the served minified build).** Two causes in `auth_gate.js`: the gate lifted before the license-token query returned, so the first AI clicks found no token; and a transient query error REMOVED the stored token, switching the AI off until the next sign-in. Now `_syncThenLift()` waits for the sync (6 s cap) at all three sign-in paths; an error keeps the last known-good token and only a positive "no valid token" removes it; `window.__slabLicenseReady` / `__slabSyncLicense` are exposed; `Provider.available()` kicks a re-sync on a miss; every guard toasts `Provider.notReadyMessage()` (sign in / Pro+ / still connecting — try again) instead of the old phrase. Lesson: a demo failure with no reproducible error is a race — read the sign-in path end to end before touching the AI code.

**PHASES, ASSUMPTIONS, ONE TABLE — see the morning entry;** all three live-verified. Assumptions page is one table (ID · Origin · Statement · Type · Credited ⇄ uncredited · Linked FCs · Rests on · State · Validation); column widths still need a pass (ID/Origin wrap, Validation clipped).

**COMMERCIAL — Radia ROM issued (`Safety_Lab_Aero_ROM_Radia_2026-09-04.docx`, SLA-ROM-001 v1.0, in the repo folder, untracked; build script lives in the session, not the repo).** Enterprise tier, 5 named engineer seats at $2,000/seat/month = $120,000 first year (read as ±25%), 2 reviewer seats at no charge (read-only, no AI), one remote OR on-site onboarding session included (travel at cost). House style now has a reference: **`SafetyLabAero_Whitepaper_AI_Guardrails_v1.0.docx` is the template for every customer document** — centred logo cover, "SAFETY LAB AERO" in blue over the title, Document control + Revision history tables, a Contents page, numbered "N  Title" headings, Arial, navy 0A1F44 / blue 0B5FA5 / text 222222 / muted 5B6472, header "Safety Lab Aero · <short title>", footer "<doc no> vX.Y · <classification> · Page N of M". Ansys medini analyze has no public price (quote-only); do not put a competitor number in a customer document.

**CAMPAIGN STATE.** Paused after run 2 (demo). Before run 3: diagnose the interdependence sweep (inspect the raw reply / colId echo in `interdep_ai.js`); items-as-MAC-members coverage in `lane_trees`; arbitration in the MAC drafter; FMEA decline reason (F13 pass); file the run 2 export; redraft the 8 sub-functions with no MAC rule. Still open with Waqas: the judgement flag on top-step Catastrophic rows (95 of 126 flagged on run 2).

## 04 Sep 2026 — Golden run 1 on v7 filed; phases are checkboxes; one assumptions table; run 2 held for the full MAC/CoFFE thread.

**GOLDEN THREAD RUN 1 (v7) FINISHED AND FILED.** `eval/runs/goldens/aeolus_g1_v7_2026-09-04T01-30.json` (functions 29, fcim 46, fha 131, ftaPages 126, pra 7, zsa 18, cma 54, assumptions 365) beside the v6 shakedown export; both committed (5f5955b). Harness is `eval/golden_thread_driver.js` (`SLGolden.thread('aeolus-gN')`, store `slab.golden.v1`, export via `runRepeatabilityExport()`). Run 1's trees were drawn by the AI synthesiser from the SDD alone — no interdependence, CoFFE or MAC behind them — which is the reason run 2 is held (below). CMA's 46 failures are `link` actions with placeholder ids (`add_cma` returns no id; the human Accept-all has the same fault) — **F13**. Declared assumptions named no condition ids, so name-only linking attached none — **F14**.

**PHASES ARE CHECKBOXES (76.45 → 76.46, live-verified).** Run 1 lost phases on 53 of 131 rows and left 16 rows with NO phases, because the model wrote "Initial climb", "takeoff", "Take-off", "cruise" and `_validPhases` matched letter-for-letter, threw the value away and put a red "1 dropped" badge on the row. Waqas: "the phases should match the mission profile and the drop down options, we should not be creating anything unique, the AI should be checking the boxes that pre-exist." Now the value is keyed on letters and digits only ("Take-off" = "Takeoff"), ticks the existing box in the profile's spelling, and a value with no box is NOT written — the row comment says "⚠ PHASE NOT IN MISSION PROFILE — the AI named X … tick the right phase"; the review card says it will not be ticked; the drafting instruction says tick existing boxes, never name a phase of your own. `droppedPhases` and the badge are gone; `unlistedPhases` replaces them. **A first cut that kept off-list values on the row as new phases was corrected before ship — the AI never adds a phase to a project.** Run 1's damaged rows stay as they are (Waqas: "those errors are good for a demo, shows how deeply our AI reads through stuff"). Lesson: the 76.45 key turned hyphens into spaces and still missed "Take-off"; a live probe of the served build after shipping is what caught it.

**SECOND FHA LAYOUT PASS SHIPPED (helpers 2.79).** Narrow pick-list columns left of Effects and for Severity; judgement badge removed from Severity; the leading "⚠ JUDGEMENT CALL" segment of Comments coloured amber (`_fhaCommentsCell`); Effects and Comments carry the width. Verified on screen.

**ONE ASSUMPTIONS TABLE (helpers 2.80, support 66.30, assumption_moat 1.4, hf_register_panel 1.8, thread_bridge 1.1, live-verified).** Waqas: "where is the actual assumption text" → "now the assumption ID is gone" → "where did the column for assumption review validation and other stuff go" → "why do we have three tables stacked? arent they all communicating the same info" → "one table is sufficient". The Assumptions page had the same 192 records three times: the working log, a 557-row self-mounted "program assumption register" (what rests on each) and a 192-row self-mounted "typed assumptions" panel, the two extras sitting ABOVE the log and pushing it 60,000 px down. Now the log is the page, with Type, Credited ⇄ uncredited (and which lane holds now), Linked conditions, Rests on (BROKEN when a dead assumption is still load-bearing; "nothing rests on it" when decorative), State, Validation. assumption_moat keeps its where-used map and INV-13/14/15 and exposes `SafetyLabAsmMoat`; the typed panel stays on the HF page only; the roster card mounts at the end of the view and only with events. `regression_asm_one_table` (17). The "557" was a double count (192 log + 365 AI ledger, 192 of which are the promoted ones) — gone with the table.

**RUN 2 IS HELD — "wait for the full MAC/CoFFE".** Waqas: "the thread should run CoFFE interdependence and MAC" before trees, because "fault trees are grounded in interdependence, coffe MAC and architecture there should be no room to hallucinate." Census of what exists: interdependence has an AI sweep (`idpAiSweep`, interdep_ai.js) but it needs systems and system functions, and a proposed cell is never a contributor until asserted; MAC has NO AI (rules are hand-built: `projectConfig.macModels[] = { id, subId, phase, clauses:[{min, of:[system function ids]}], substantiation:{kind, ref, by, at} }`); CoFFE is computed from MAC + interdependence (`projectConfig.coffe.verdicts['<fcInternalId>§<caseKey>']`), its residue signed by hand; trees can be COMPILED from all three plus CRA by `SLLaneTrees.compileAll()` (lane_trees.js) — run 1 used the AI synthesiser instead. Build order, register **F15**: (1) systems + system functions through the harness; (2) interdependence sweep as a thread step, harness asserts proposals by model (testing only — "accept without a click is just for testing"); (3) MAC drafter, new lane, eval-gated; (4) CoFFE residue proposer, new lane; (5) trees compiled, replacing `synthesizeTree` in the thread. Consistency bar for trees: logic identical run to run, node prose close on meaning.

**STILL OPEN WITH WAQAS.** The judgement flag on rows whose Catastrophic comes from the agreed top step (100 of 131 rows flagged on run 1) — flag stays as-is until he rules. Assumptions-log widths: ID wraps to three lines, Origin one word per line, Validation clipped — offered, not yet approved.

## 03 Sep 2026 (evening) — five weeks of work put under version control; the campaign measured, and six of its readings withdrawn.

**THE REPO HAD NOT BEEN COMMITTED SINCE 30 JULY, and nobody knew because deploying never needed it.** `ship.sh` is wall → build → smoke gate → `npx wrangler deploy`; it contains no git at all, so every deploy this month shipped correctly from the working tree while the history stood still. Root cause found by reading rather than guessing: a zero-byte `.git/index.lock` dated **31 July**, which made every `git add` since fail against it — and the staged tree was therefore a five-week-old snapshot that read as a 2,139-line deletion. `_to_delete/` already held `stale-git-lock-30jul` and `-b`, so this had happened before and been worked around rather than diagnosed. The mount forbids unlink, so git could not clear its own locks; delete permission was requested and granted for the folder, and locks now clean up by themselves. Committed in reviewed batches — 205 files under site/, 286 tests+build, 129 eval+docs, worker.js — after a content scan for secrets (the one `sk-ant-api03…` hit is the placeholder text in the key input; worker.js reads UPLOAD_TOKEN from env, never source). ~59 MB of dated backup trees excluded via .gitignore. **Waqas pushed back on the claim and was right to — deploying and committing are different operations, and saying "nothing has been committed" without that distinction sounded like a claim his deploys had not gone out.** Still true and still open: there is **no remote**, so this is rollback, not backup.

**SHIPPED AND LIVE-VERIFIED: 76.38 (picker guards) and 76.39 (`_captureBail`).** Both proven with a byte-size control probe, not a bare 200 — the real module serves as text/javascript at 638,020 B while a fabricated filename also returns 200 with 346,219 B of text/html. Note for future probes: **the build minifies identifiers and normalises to double quotes**, so a served-file check must look for string literals, not function names; checking for `function _captureBail` returned false on a build that contains it.

**F3 CLOSED — a refusal is a result (76.39).** The 3 Sep campaign queued three FMEA runs; all three burned 901 s and reported "no panel opened". The lane had refused in milliseconds — `ppfmea` is absent from that project's config, so the opt-in scope gate turned it away before any model call — and said so into a toast nobody was listening to. (First reported here as a provider outage; that came from probing `window.Provider`, which is not a global, so the check could not distinguish "down" from "not visible". The scope-gate cause is read from the saved project. The tool refused CORRECTLY — 21 systems and 65 trees were available — and the only defect was that a correct refusal was invisible.) **99 toast-and-return guard clauses in ai_assistant.js resolved no capture**, so any lane refusing early hung a campaign for its full timeout. Rather than touch 99 sites, `_captureGuard` wraps the 17 public lane entry points: armed, it awaits the lane and, if nothing fired the seam, resolves with declined + bailed + the reason + `saidToUser` (`_toast` now remembers its last message). Inert when disarmed — the engineer's path never enters that branch. Also guards `_fmeaSystemPicker`, **the sixth picker**: the 76.38 sweep matched `_open*Picker` and missed it, which is precisely how FMEA came to hang. `regression_capture_bail` — 30 checks, 14 executed. Four suites asserted the export by literal text and were updated to accept the wrapper while still pinning the same function behind it. **Wall hygiene finding:** `regression_hf_lane_drafters` prints `FAIL` with one space where ship.sh greps `^  FAIL  ` with two, so a real failure was scored as a *crash* — harmless here, mislabels a genuine defect in general.

**THE CAMPAIGN, MEASURED — and the principal finding is directional.** Store exported to `eval/runs/slab_campaign_v1_2026-09-03.json` (15 runs, 12 landed, 573 rows) and **committed before scoring**, per rule 26. Full write-up in `eval/FHA_CONSISTENCY_RESULTS_2026-09-03.md`. Opus held scope perfectly on the 85 supplied conditions (85/85/85, identical row count per condition, zero mismatches) where Sonnet returned 3/5/3 on a 3-condition scope. Pairwise agreement effAc 0.827 / effCrew 0.792 / effPax 0.932. Counts: Catastrophic 17/16/18 — reproducible to ±1 — while Major swings 12/21/20 and Minor 10/2/4. Severity blank on **exactly 37 rows every run** with only 21 blank in all three of 51 ever-blank (stability 0.41): a pinned total over shifting membership, which looks like a quota rather than per-row uncertainty and is the cheapest outstanding check. **The finding that matters: against the project's own 2 Sep accepted rows, disagreement breaks 18 less severe to 2 more severe.** Noise is symmetric; this is drift, it sits in the middle band, and under-classification is the direction that removes DAL and verification rigour.

**SIX READINGS TAKEN EARLIER THE SAME SESSION WERE WRONG AND ARE WITHDRAWN IN THE WRITE-UP.** (1) Severity reported as a fourth independent axis — it is `max()` of the other three, so the perfect Catastrophic↔hull-loss pairing (51/51 both directions) is **tautological**, and the "joint top step is working" reading from the Vayu run cannot be evidenced this way. (2) Golden runs first scored by row id — but `FC-002`/`FC-003` are sequential slot numbers and the lists shift by one, so different failure conditions were being compared; re-scored by content, overlap is 38–66% and two runs that both produced ~100 rows share only 50 conditions and agree on severity half the time. **Matching counts hid a different analysis — count similarity is not a selection criterion.** (3) "Found all 85 every time" quoted as consistency: **the 85 were supplied as input**, so that is echoing a list, and the enumeration step — the hard half — was never tested. (4) The project's 253 rows called "the engineer's answer": all 253 are `aiGenerated: true`, zero human-authored, so that comparison is Opus-2-Sep vs Opus-3-Sep, not accuracy. (5) Sonnet's extra rows called "invented" — they are real project conditions drafted out of scope. (6) All seven golden files are AI-generated throughout; **nothing in this repo is an answer key.**

**RULINGS FROM WAQAS.** (a) **The bar is that the intent of the effect is captured and the severity is correct; identical wording is a bonus, not the metric.** Token overlap sees neither, so every wording figure is now reported as a diagnostic and F4 says plainly not to quote it as a consistency result again. (b) **The tool must generate the failure conditions itself** — handing them over removes the part of the job that matters. (c) On aircraft-level phrasing: *"complete loss of propulsive thrust"* is correct at aircraft level; *"complete loss of thrust generation"* hints at loss of engine, whereas loss of thrust may arise on the **inceptor, computation or effector** side, and those three are what feed interdependence — the skill's own IMPLEMENTATION-AGNOSTIC rule already forbids the drift, and word overlap scored it as a rephrase. (d) The `sevBasis` pill backfill was **dropped as admin** after checking: every `effAcLevel` reference is in the parse/apply/display path and none feeds existing rows back into the drafting prompt, so it would light chips on a test project and change nothing. (e) Standing preference: **plain language, no jargon, in every response.**

**PILLS — NOT A BUG.** All 253 rows in HF Consistency · Aeolus · D1 carry severity and `sevBasis` and **zero** effect levels, so the chips have nothing to draw. Every one was accepted 2 Sep 19:34–20:05 UTC; the levels shipped ~9 h later on 3 Sep. The accept path does write all three levels (`_applyFhaSuggestion`) — it simply has not run since. **QUEUE:** confirm the abstention quota; extend axis coherence below the top step and require the independent-means count; re-run enumeration properly (tool finds its own conditions, 3 runs, content-matched) — the only honest before/after; a hand-classified 20-condition answer key, without which "accuracy" stays unusable; an intent-level judge to replace token overlap; the HF-schema scorer (tid 43/51/43 and task 62/55/53 landed clean but are unscored); then the older items — phase profile by cert basis, the silent phase-drop flag, Stage B on the action path, cloud-load-beats-local-state, Table A6 anchors flagged by the checker, and a git remote.
