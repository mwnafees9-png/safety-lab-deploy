...
**NEXT (nothing started; each build stated back first).** Fix the six criticals, then the HIGH list, then the Enterprise builds already agreed (unified activity log incl. AI calls; one classification label on every channel; retention + legal hold + complete destruction certificate; workspace admin controls; AI data receipt; customer-managed keys last), then SL-WP-0003 Rev 3.0 and the trust/security pages rewritten to match. Audit tarballs removed from Downloads.
