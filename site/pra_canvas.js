...
// set (PRA_LIBRARY), lets the analyst disposition each (Applies / N/A + reason),
// and — for the applicable ones — captures the FOOTPRINT as a set of zones over
// the zonal model. Writes praData rows with a structured `zones` field, which
// PHYS_CROSSCHECK.praFootprints() reads to fire the PRA co-location cross-check.
//
// This closes the CCA loop: PRA (here) + ZSA (walkthrough) + CMA (walkthrough)
// all author into the same physical-independence engine.
//
// Honesty boundary: the library is a deterministic CANDIDATE set; applicability
// and the footprint are AUTHORED (human judgement). The intersection against
// credited-independent members is then computed.
// ============================================================================
(function () {
    'use strict';
    function _esc(s) { if (typeof esc === 'function') return esc(s); return String(s == null ? '' : s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;'); }
    function _pra() { if (typeof praData === 'undefined') { window.praData = window.praData || []; return window.praData; } return praData; }
    function _Z() { return window.ZONES; }
    function _save() { try { if (typeof scheduleAutosave === 'function') scheduleAutosave(); } catch (_) {} try { if (typeof renderPRA === 'function') renderPRA(); } catch (_) {} }
    function _rowId() { return (typeof newRowId === 'function') ? newRowId() : ('pc-' + Date.now() + Math.random().toString(36).slice(2, 6)); }
    function _aid() { return (typeof _newAnalysisId === 'function') ? _newAnalysisId('PRA') : ('PRA-' + Math.floor(Math.random() * 9000 + 1000)); }
    function _certBasis() { var pc = (typeof projectConfig !== 'undefined' ? projectConfig : {}) || {}; return pc.regulation === 'Part 23' ? ('Part 23 ' + (pc.part23Class || 'IV')) : (pc.regulation || 'Part 25'); }
    ...
    window._praSetDisp = function (libId, disp) {
        ...
        if (disp === 'applies') {
            ...
            else { row.disposition = 'na'; row.naReason = reason; row.zones = []; row.affectedZones = []; }
        } else { // candidate -> remove the row
            if (row) { var a = _pra(); var i = a.indexOf(row); if (i >= 0) a.splice(i, 1); }
        }
        _save(); _renderModal();
    };
    window._praAddZone = function (libId, sel) {
        if (!sel || !sel.value) return; var row = _row(libId); if (!row) return;
        if (row.zones.indexOf(sel.value) < 0) { row.zones.push(sel.value); row.affectedZones = row.zones.map(_zoneCode); }
        _save(); _renderModal();
    };
    window._praRemoveZone = function (libId, zid) {
        var row = _row(libId); if (!row) return;
        var i = row.zones.indexOf(zid); if (i >= 0) row.zones.splice(i, 1); row.affectedZones = row.zones.map(_zoneCode);
        _save(); _renderModal();
    };
    window._praCloseCanvas = function () { var m = document.getElementById('pra-canvas-modal'); if (m) m.remove(); };
    ...
    function _renderModal() {
        var m = document.getElementById('pra-canvas-modal'); if (!m) return;
        var cand = window.PRA_LIBRARY.candidates(_certBasis());
        var body = '<div style="font-size:11.5px;color:#55555C;margin-bottom:6px;">' + _esc(cand.rigor) + '</div>' +
            cand.items.map(function (lib) {
                var row = _row(lib.id);
                var disp = row ? row.disposition : 'candidate';
                var opt = [['candidate', '·', '#8a8a8a'], ['applies', 'Applies', '#8E2A2A'], ['na', 'N/A', '#1E7A34']];
                var btns = opt.map(function (o) {
                    var on = disp === o[0];
                    return '<button onclick="_praSetDisp(\'' + lib.id + '\',\'' + o[0] + '\')" style="font-size:10px;padding:1px 8px;border:1px solid ' + o[2] + ';border-radius:5px;cursor:pointer;margin-left:3px;' + (on ? ('background:' + o[2] + ';color:#fff;') : ('background:transparent;color:' + o[2] + ';')) + '">' + o[1] + '</button>';
                }).join('');
                var detail = '';
                if (disp === 'applies') detail = '<div style="font-size:11px;color:#55555C;margin-top:2px;">Footprint hint: ' + _esc(lib.footprint) + '</div>' + _zoneChips(lib.id, row) +
                    (row.zones && row.zones.length >= 1 ? '' : '<div style="font-size:10.5px;color:#8E2A2A;margin-top:2px;">Add the zones this risk’s footprint spans to enable the cross-check.</div>');
        ...
        m.querySelector('#pra-canvas-body').innerHTML = body;
    }

    window.praCanvas = function () {
        window._praCloseCanvas();
        var ov = document.createElement('div'); ov.id = 'pra-canvas-modal';
